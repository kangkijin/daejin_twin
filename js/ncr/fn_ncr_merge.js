...
function fnSatisOnOff(satRschType, obj) {
	...
	if (satRschType == "std") {
			...
			scaleQusArry += ',본 프로그램을 적극 추천해주고 싶습니까?';
//			scaleQusArry += ',어떤 경로를 통해 참가하시게 됐습니까?';
			scaleQusArry += ',본 프로그램 명칭이 교육 내용에 적합하다고 생각하십니까?';
			scaleQusArry += ',본프로그램에서 다뤄진 내용들은 본인에게 유용한 정보와 지식이었습니까?';
			scaleQusArry += ',본 프로그램의 참여시간은 적절하였습니까?';
			scaleQusArry += ',본 프로그램의 교육 내용이 체계적이었습니까?';
			scaleQusArry += ',본 프로그램과 관련한 교육시설 및 제반여건은 적절하였습니까?';
			scaleQusArry += ',본 프로그램을 통하여 나의 능력(지식/기술/태도)이 향상 되었다고 생각하십니까?';

			choiceQusArry = '본 프로그램에의 참여가 우리 대학 핵심역량 중 어느 역량 향상에 도움이 되었다고 생각하십니까?';
			choiceQusArry += ',어떤 경로를 통해 참가하시게 됐습니까?';

			choiceAnsQusArry +=  '1^인성역량';
			choiceAnsQusArry += ',1^혁신역량';
			choiceAnsQusArry += ',1^글로벌역량';
			choiceAnsQusArry += ',1^소통역량';
			choiceAnsQusArry += ',1^자기개발역량';
			choiceAnsQusArry += ',1^창의융합역량';

			choiceAnsQusArry += ',2^교직원 추천';
			choiceAnsQusArry += ',2^친구/선후배 추천';
			choiceAnsQusArry += ',2^오프라인 홍보물';
			choiceAnsQusArry += ',2^온라인 홍보(홈페이지/문자/T-Win)';
...
function addBudget() {
	...
	$tableTr.find("tbody").append(
					'<tr id="budgetTr_'+trIndex+'">'
						...
						+'<select class="form-control input-sm emptyChkByClass dp_inline wd_p40 mg_r5" name="nbiMainBudgetCd" id="nbiMainBudgetCd" title="예산 대분류" onchange="fnCreateBudgetEtc(this);">'
							+'<option value="">예산대분류를 선택해주세요.</option>'
							+'<option value="NCR_T09_P01">국고</option>'
							+'<option value="NCR_T09_P02">대응</option>'
							+'<option value="NCR_T09_P03">교비</option>'
					...
					+'<td>'
					+'<div class="pd_t5" id="budgetSubCdDiv_'+trIndex+'">'
						+'<select class="form-control input-sm emptyChkByClass wd_p40 dp_inline mg_r5" name="nbiBudgetCenCd" id="nbiBudgetCenCd" title="예산 중분류" onchange="fnCreateBudgetEtc(this)">'
							+'<option value="">예산중분류를 선택해주세요.</option>'
							+'<option value="NCR_T10_P01">등록금 회계</option>'
							+'<option value="NCR_T10_P02">비등록금 회계</option>'
							+'<option value="NCR_T10_P99">기타</option>'
						+'</select>'
					...
					+'<td>'
					+'	<div class="pd_t5">'
					+'		<input type="text" class="form-control ta_r input-sm dp_inline va_t emptyChkByClass" onkeyup="chkByte(this, 1000)" name="nbiBudgetThirdText" id="nbiBudgetThirdText" title="소분류"  placeholder="예산 소분류를 작성해주세요">'
					+'	</div>'
					+'</td>'

					+'<td>'
						+'<div class="pd_t5" id="budgetPriceDiv_'+trIndex+'">'
...
function addSubBudget(_obj) {
	...
	$(_obj).parent('div').parent('td').prev().prev().append(
			'<div class="pd_t5" id="budgetCenCdDiv_'+subIndex+'">'
			+'<select class="form-control input-sm emptyChkByClass wd_p40 dp_inline mg_r5" name="nbiBudgetCenCd" id="nbiBudgetCenCd" title="예산 중분류" onchange="fnCreateBudgetEtc(this)">'
			+'<option value="">예산중분류를 선택해주세요.</option>'
			+'<option value="NCR_T10_P01">등록금 회계</option>'
			+'<option value="NCR_T10_P02">비등록금 회계</option>'
...
function removeBudget(_obj,_subYn) {
	if (_subYn == 'sub') {
		...
		$(_obj).parent("div").parent("td").prev().prev().children("div #budgetCenCdDiv_"+index[1]).remove();
...
function fnChangeAreaActCode (obj) {
	...
	if (_areaId == 'npiAreaCd') {
		fnSetComnCdCombo(_areaActCode, 'npiAreaSubCd', '','선택', true, '' ,'Y');
...
function fnOnClickSave(viewType){
	...
	if( viewType == 'MODIFY'){
		...
		if('${programInfoMap.NPI_STAT_DB_CD}' == 'NCR_T05_P06' ||'${programInfoMap.NPI_STAT_DB_CD}' == 'NCR_T05_P07'||'${programInfoMap.NPI_STAT_DB_CD}' == 'NCR_T05_P08'){
			var roleId = "${sessionScope.loginVO.roleId}";

			if(roleId.includes("RLE0000024") || roleId.includes("SUPERADMIN_01")) {
	...
	/** 해시태그처리 **/
	var tag_val = "";
	$("#tag-list input[name='tag']").each(function(x,y) {
		if(tag_val == ""){
			tag_val += y.value;
		}else if(tag_val != "" ){
//				tag_val += ","+"'"+y.value+"'";
...
//		if(tag_val!="")tag_val = tag_val.substring(1);
	$("#npiTagData").val(tag_val);

	/** 신청서필수여부처리 **/
	if($("#npiApplyMustY").is(":checked") == true){
		$("#npiApplyMustYn").val('Y');
	}else{
		$("#npiApplyMustYn").val('N');
	}

	/*첨부파일 존재여부*/
	if($("#fileDownRow .thumbnail_img").length > 0){
		$("#npiFileYn").val('Y');
	}else{
		$("#npiFileYn").val('N');
	}
...
function fnInputValidation() {
	...
	if(nullValidation){
		...
		/** 담당부서 체크 **/
		if ($('#npiOprtDeptCd').val() == null || $('#npiOprtDeptCd').val() == '') {
			alert("담당부서를 검색하신 후 선택해 주시기 바랍니다.")
			nullValidation = false;
			$('input[name=npiOprtDeptNm]').focus();
			return false;
		}

		/** 담당자 코드 체크 **/
		if ($('#npiStfNo').val() == null || $('#npiStfNo').val() == '') {
			alert("담당자를 검색하신 후 선택해 주시기 바랍니다.")
			nullValidation = false;
			$('input[name=npiStfNm]').focus();
			return false;
		}

		/** 신청서 Y 체크했을경우 파일 등록 유무 체크 **/
		if ($('input[name="npiApplyFileYn"]:checked').val() == "Y") {
			if ($('#regFileTr .file_thumbnail').length <= 0) {
...
//				$("#npiPreRschN").trigger('click');
		...
		if( $('input[name=npiSatisRschYn]:checked').val() == "Y"){
			if(! fnChkSatisNullchk('std')){
				return false;
			}else if($("#npiSatStrDate").val() == '' || $("#npiSatEndDate").val() == ''){
				alert("만족도 조사 기간을 설정해야합니다.");
				nullValidation = false;
				return false;
			}

			if(!$("#npiSatStrDate").val() == "" && !$("#npiSatEndDate").val() == ""){
				if($("#npiSatStrDate").val() > $("#npiSatEndDate").val()){
					alert("만족도 조사 시작일을 종료일보다 빠르게 설정하실수 없습니다.");
					return false;
				}
			}
		}
		...
		/** 계획성 평가 체크 **/
		for (var i = 1; i <= 10; i++) {
			if( isEmpty($('input[name=npePlanEvalQus'+i+']:checked').val())){
				alert(i+'번째 계획성 평가 문항에 답변해 주세요.');
				$('input[name=npePlanEvalQus'+i+']').val().focus();
				return false;
			}
		}
...
@ param    : stateMessage
@ history  : 2017-11-07 (최초작성)
@ history  : 2022-03-22 (로그 추가)
**********************************************************
**/
function fnShowResult(response, stateMessage) {
	if (response.isSuccess) {
		alert('프로그램 저장에 성공하였습니다.');
		console.log("[SUCCESS LOG] : " + "response : " + response + "stateMessage : " + stateMessage);
		fnGoNcrProgramListAdmin(response.programGubun, response.viewType);
	} else {
		alert('프로그램 저장에 실패하였습니다.(2)');
		console.log("[UNSUCCESS LOG] : " + "response : " + response + "stateMessage : " + stateMessage);
	}
}
/**
*********************************************************
		...
		//저장일때
			...
			if(programGubun.indexOf('ncr') !== -1){
				if (programGubun.indexOf('ApprovSTF') !== -1) {
					location.href = basePath+"/ncrProgramApprovSTF/r/m/getProgramList.do";
				} else {
					location.href = basePath+"/ncrProgramListSTF/r/m/getProgramList.do";
				}
			}
...
function fnGoModiDetailInfo(_programGubun,_keyId,_viewType){
	...
	_url += "?npiKeyId=" + encodeURI(encodeURIComponent(_keyId))+"&viewType="+encodeURI(encodeURIComponent(_viewType));
...
@ param    : _programGubun = 프로그램 구분
@ param    : _keyId = 비교과 프로그램 키값
@ param    : _maxCnt = 비교과 프로그램(상담) 인정횟수
@ history  : 2019-05-20 (최초작성)
****************************************
**/
function fnGoRecrListAdd(_programGubun,_keyId,_maxCnt) {
	$('#detailForm #npiKeyId').val(_keyId);
	$('#detailForm #npiMaxAckCnt').val(_maxCnt);
...
function fnSortAddFavorite() {
	...
	if(loginUserId == ''){
		alert("찜 목록 보기는 로그인이 필요한 서비스 입니다.");
		$("#sortFavorite").prop("checked", false);
		return false;
...
@ comment  : 리스트 검색
@ history  : 2019-05-10 (최초작성)
2022 05 02 맹준호 :: 동료평가 리스트 사라져서 url => myView로 통일
**********************************************************
**/
function searchProgram() {
	...
	var addImpossible =$("#addImpossible").val();
	var _programGubun = $('#programGubun').val();
	var diagFlag = $('#diagFlag').val();
	var divType = $("#divType").val();
	var npiType = $("#npiType").val();
	var npiSubType = $("#npiSubType").val();
	var aiRecomYn = $("#aiRecomYn").val();
	var hashTagWord = $("#hashTagWord").val();
	var npiStateCd = $("#npiStateCd").val();

	var _url = BASE_PATH+"/myView/a/m/getProgramApplListAjax.do";
	...
	$.ajax({
		...
		, data : {
			...
			addImpossible : addImpossible,
			npiTitle : npiTitle,
			diagFlag : diagFlag,
			divType : divType,
			npiType : npiType,
			npiSubType : npiSubType,
			aiRecomYn : aiRecomYn,
			hashTagWord : hashTagWord,
			npiStateCd : npiStateCd
...
function fnGoApplyList(_viewType) {
	...
	if (_viewType == 'MODIFY') {
		$('#listForm input[name=listType]').val('myhistory');
		$("#listForm").attr("action", BASE_PATH+"/myView/a/m/getMyViewList.do").submit();
	}else {
		$('#listForm input[name=listType]').val('nunsubject');
		$("#listForm").attr("action", BASE_PATH+"/myView/a/m/getMyViewList.do").submit();
	}
}
...
function fnDetailViewAblityDountGraph(ablityScoreArry, categoriesArry) {
 var options = {
        ...
        colors: ['#d93655','#9dcd57','#0487d9','#f2b84c','#855ea8','#088A85','#FA58F4','#000000'],
        responsive: [{
          breakpoint: 480,
          options: {
            chart: {
      ...
      var chart = new ApexCharts(document.querySelector("#ablityDountGraphDiv"), options);
      chart.render()
...
function fnCalMileageScore() {
	...
	if (npiCounselYn == "Y") {
		if(isEmpty(npiStanMileScore)){
			...
			$("#abilitySumValTag").text(npiStanMileScore*100+"점");
		...
		if(isEmpty(npiPartyTime)){
			...
			$("#abilitySumValTag").text(parseInt(npiPartyTime*calcNpiLevel*100)+"점");
...
//	}
	...
	if (obj.value != "") {
		...
		if(1 < (parseInt(pTagRowCnt.length)+parseInt(1))){
			alert("대표이미지는 1개 까지 가능 합니다.");
			obj.value = "" ;
			return false;
		}
...
//				$('#'+obj.name+'_idx').parents("p").append(addFileInputHtml);
	...
	**/
	...
	function fnSetClogForSustCdCheckBox(clogCd, viewId, chkdValue, titleValue, valueName, useAsync,viewType)  {
		var basePath = sessionStorage.getItem("contextRootPath");
					...
					for(var idx=0; idx<r.beanlist.length; idx++) {
						...
						if(viewType == 'MODIFY'){
							...
							if($("#"+viewId+idx).is(":checked")){
								targetView.append('<input type="hidden" name="npiPartiDeptNm" value="'+obj.DEPT_KOR_NM+'">');
							}
						}else{
							targetView.append(
									'<p class="ad_input_row">'
	...
	**/
	...
	function fnSetUpNcrProgramCode(upCode, selValue) {
		...
		var progracmCnt = fnGetNcrProgramSubCode(upCode, "npiProgramCdSel", selValue, '중위프로그램을 선택해주세요', false);
		// 중위 코드 있으면 유효성 추가
...
//		}
	...
	function fnSetNcrProgramTitle(programCd, selValue) {
		...
		var progracmCnt = fnGetNcrProgramTitle(programCd, "npiProgramTitleSelCon", selValue, '프로그램명을 선택해주세요', false);
	}
	...
	**/
	...
	/**
		...
		**********************************************************
	**/
	function fnGetNcrProgramTitle(comnCdPcodeId, viewId, selValue, allValue, useAsync)  {
		var basePath = sessionStorage.getItem("contextRootPath");
		...
		jQuery.ajax({
			...
			success: function(r) {
				if (r.beanlist.length > 0) {
					for(var idx=0; idx<r.beanlist.length; idx++) {
						var obj = r.beanlist[idx];
						if(typeof selValue != "undefined" && selValue == obj.NPI_KEY_ID) {
							targetView.append("<option value='"+obj.NPI_KEY_ID+"' selected>"+obj.NPI_TITLE+"</option>");
						} else {
							targetView.append("<option value='"+obj.NPI_KEY_ID+"'>"+obj.NPI_TITLE+"</option>");
						}
					}
				}
			},
...
function fnGetNcrProgramSubCode(comnCdPcodeId, viewId, selValue, allValue, useAsync)  {
			...
			if (r.beanlist.length > 0) {
				for(var idx=0; idx<r.beanlist.length; idx++) {
					...
					if(typeof selValue != "undefined" && selValue == obj.PSC_SUBJ_CD) {
						targetView.append("<option value='"+obj.PSC_SUBJ_CD+"' selected>"+obj.PSC_SUBJ_NM+"</option>");
					} else {
						targetView.append("<option value='"+obj.PSC_SUBJ_CD+"'>"+obj.PSC_SUBJ_NM+"</option>");
					}
				}
			}
		},
	...
	**/
	function fnSetProgramCode(porgramCd) {
		...
		if (npiCodeType == "NEW") {
			...
			var _url = BASE_PATH+"/ncrProgramRegSTF/r/n/getNcrProgramCousSubInfo.do";
			$.ajax({
				url : _url
				, type : "post"
					, async : false
				...
				, success : function(res) {
					if(res.ncrProgramCodeInfoMap != null){
						// 1. 프로그램 영역 세팅
						/*$("#npiAreaCd").val(res.ncrProgramCodeInfoMap.PSC_AREA_CD);
						$("#npiAreaCd").trigger("change");*/
						// 2. 프로그램 구분 세팅
						$("#npiType").val(res.ncrProgramCodeInfoMap.PSC_TYPE);
						$("#npiSubType").val(res.ncrProgramCodeInfoMap.PSC_SUB_TYPE);
						$("#npiType").trigger("change");
						// 3. 상담 비교과 여부
						if(res.ncrProgramCodeInfoMap.PSC_COUNSEL_YN == 'Y'){
							$("#npiCounsel_Y").trigger("click");
							$("#npiStanMileScore").val(res.ncrProgramCodeInfoMap.PSC_STAN_MILEAGE);
							$("#npiMaxAckCnt").val(res.ncrProgramCodeInfoMap.PSC_MAX_ACK_CNT);
						}else{
							$("#npiCounsel_N").trigger("click");
							$("#npiPartyTime").val(res.ncrProgramCodeInfoMap.PSC_PARTI_TIME);
							$("#npiLevel").val(res.ncrProgramCodeInfoMap.PSC_LEVEL);
						}
						fnCalMileageScore();
						// 4. 역량 퍼센트 세팅
						$(".ability").val('');
						$(".ability").each(function(idx){
							var abilityCd = "PSC_A00"+(idx+1);
							$(this).val(res.ncrProgramCodeInfoMap[abilityCd]);
							fnCalAbilityScore(this);
						});
						// 5. 모집인원 세팅
						$("#npiRecrCnt").val(res.ncrProgramCodeInfoMap.PSC_RECR_CNT);
						// 6. cont 세팅
						CKEDITOR.instances["npiNeedsCont"].setData(res.ncrProgramCodeInfoMap.PSC_NEED_CONT)
						CKEDITOR.instances["npiEduCont"].setData(res.ncrProgramCodeInfoMap.PSC_EDU_CONT)

						$("#npiApplPrecautions").val(res.ncrProgramCodeInfoMap.PSC_APPL_PRECAUTION);
						$("#npiSupBenefit").val(res.ncrProgramCodeInfoMap.PSC_SUP_BENEFIT);
						$("#npicontEtc").val(res.ncrProgramCodeInfoMap.PSC_ETC_CONT);
					}
				}
			});
		}
	...
	**********************************************************
	...
	function addConRcrProgram(rcrProgramCd, rcrPrrogramCdNm) {
		...
		if (isEmpty(rcrProgramCd)) {rcrProgramCd = "";};
		if (isEmpty(rcrPrrogramCdNm)) {rcrPrrogramCdNm = "";};


		conRcrProgramTag = '<div name ="conRcrProgramDiv" id="conRcrProgramDiv_'+_rowCount+'">';
		conRcrProgramTag += '<p class="ad_search_row mg_t10" id="conRcrProgramP_'+_rowCount+'" >';
		conRcrProgramTag += '<input type="text" class="form-control" name="npiConRcrProgramCodeNm" id="npiConRcrProgramCodeNm_'+_rowCount+'" placeholder="검색어를 입력하세요" title="연계교과목" value="'+rcrPrrogramCdNm+'">';
		conRcrProgramTag += '<button type="button" class="btn btn-primary btn_form btn_search" onclick="fnOpenConRcrProgramPopUp('+"'"+ _rowCount+ "'"+ ');"><i class="fa fa-search"></i>검색</button>';
		conRcrProgramTag += '<input type="hidden" class="ad_formstyle" name="npiConRcrProgramCode"  id="npiConRcrProgramCode_'+_rowCount+'" value="'+rcrProgramCd+'">';
		conRcrProgramTag += '</p>';
	...
	function removeConRcrProgram() {
		...
		var childCount = typeof conRcrProgramDiv == "undefined" ? 0 : conRcrProgramDiv.length;
	...
	**********************************************************
	...
	function fnOpenConRcrProgramPopUp(idx) {
		...
		var popupParams = {idx:idx	}; // 팝업 호출시의 파라미터업 호출시의 파라미터
		loadPopup(popupDivId, popupUrl, popupParams);
	}
	...
	**********************************************************
	...
	function addConNcrProgram(ncrProgramCd, ncrPrrogramCdNm) {
		...
		if (isEmpty(ncrProgramCd)) {ncrProgramCd = "";};
		if (isEmpty(ncrPrrogramCdNm)) {ncrPrrogramCdNm = "";};


		conNcrProgramTag = '<div name ="conNcrProgramDiv" id="conNcrProgramDiv_'+_rowCount+'">';
		conNcrProgramTag += '<p class="ad_search_row mg_t10" id="conNcrProgramP_'+_rowCount+'" >';
		conNcrProgramTag += '<input type="text" class="form-control" name="npiConNcrProgramCodeNm" id="npiConNcrProgramCodeNm_'+_rowCount+'" placeholder="검색어를 입력하세요" title="연계비교과목" value="'+ncrPrrogramCdNm+'">';
		conNcrProgramTag += '<button type="button" class="btn btn-primary btn_form btn_search" onclick="fnOpenConNcrProgramPopUp('+"'"+ _rowCount+ "'"+ ');"><i class="fa fa-search"></i>검색</button>';
		conNcrProgramTag += '<input type="hidden" class="ad_formstyle" name="npiConNcrProgramCode"  id="npiConNcrProgramCode_'+_rowCount+'" value="'+ncrProgramCd+'">';
		conNcrProgramTag += '</p>';
	...
	function removeConNcrProgram() {
		...
		var childCount = typeof conNcrProgramDiv == "undefined" ? 0 : conNcrProgramDiv.length;
	...
	function fnSetCalcPlanScore() {
		...
		for (var idx = 1; idx <= 10; idx++) {
			if (isEmpty($('input[name="npePlanEvalQus'+idx+'"]:checked').val())) {
				...
				tempScore = $('input[name="npePlanEvalQus'+idx+'"]:checked').val();
			}
		...
		$("#planQusAvgTd").text((parseInt(calScore) / parseInt(10)).toFixed(1)+"점");
		$("#npeAvgScore").val((parseInt(calScore) / parseInt(10)).toFixed(1));
	}
	...
	****************************************
	...
	function fnAddNcrTeamCdList(teamNm , teamCd) {
		...
		var teamInputTag ="";
		if (isEmpty(teamNm)) {teamNm = '';}
		if (isEmpty(teamCd)) {teamCd = '';}

		teamInputTag +=	'<div class="col-xs-4 mg_t10"  id="ncrTeamDivTag_'+_rowCount+'" >';
		teamInputTag += '	<div class="removable_input">';
		teamInputTag +=	'		<input type="text" class="form-control" name="ncrTeamNm" id="ncrTeamNm_'+_rowCount+'" placeholder="" value="'+teamNm+'">';
		teamInputTag +=	'		<input type="hidden" name="ncrTeamKeyId" id="ncrTeamKeyid_'+_rowCount+'" value="'+teamCd+'">';
		teamInputTag +=	' 		<button type="button" onclick="fnDelNcrTeamCodeInfo('+"'"+teamCd+"','"+_rowCount+"'"+')" >x</button>';
		teamInputTag +=	'	</div>';
		teamInputTag +=	'</div>';
	...
	****************************************
	...
	function fnOpenNcrHisPopUp(npiKeyId, npsStdNo) {
		...
		var popupDivId = "#layerPopup"; // 팝업이 들어가는 div의 id
		var popupUrl = BASE_PATH+"/"+_programGubun+"/r/n/goNcrProgramHisListPopUp.do";
		var popupParams = {
				 npiKeyId : npiKeyId
				,npsStdNo : npsStdNo
		} // 팝업 호출시의 파라미터
	...
	****************************************
	...
	function fnOpenCounselHisPopUp(npiKeyId, npsStdNo) {
		...
		var popupDivId = "#layerPopup"; // 팝업이 들어가는 div의 id
		var popupUrl = BASE_PATH+"/"+_programGubun+"/r/n/goCounselHisListPopUp.do";
		var popupParams = {
				 npiKeyId : npiKeyId
				,npsStdNo : npsStdNo
		} // 팝업 호출시의 파라미터
	...
	**********************************************************
	...
	function fnApplyStdStateGraph(divNm, dataArry, categoriesArry) {
		var options = {
				...
				colors: ['#37aa82', '#d7bee1', '#e9884e', '#ffcd7f', '#faa7d2', '#5087be'],
				fill: {
					colors: ['#37aa82', '#d7bee1', '#e9884e', '#ffcd7f', '#faa7d2', '#5087be'],
				},
				plotOptions: {
