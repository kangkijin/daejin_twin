...
//							+"<span class='stit_bullet'></span>자유형 객관식 답변 작성"
	...
	if( type == "pre" ) {
		...
		if(_val == "SCALE") {
			...
			$tableTd.prev().empty();
			// 이전요소(질문정보가 들어 있는 td)질문 내용 넣기
			$tableTd.prev().append("<input type='text' class='form-control input-sm' name='sqmCont2' title='질문 내용' onkeyup='fnKeyUpCommaChk(this);'>");
...
//							+"<span class='stit_bullet'></span>자유형 객관식 답변 작성"
						...
						+"<ul>"
							+"<li class='ta_l mg_t5'>"
								+"<input type='text' class='form-control input-sm dp_inline wd_p90 va_m' name='samCont2' title='답변 내용' onkeyup='fnKeyUpCommaChk(this);'>"
